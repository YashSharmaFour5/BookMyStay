...
function Card({ title, landMark, image1, image2, image3, rent, city, id, ratings, isBooked, host }) {
    ...
    return (
        <div className='w-[330px] max-w-[85%] h-[460px] flex items-start justify-start flex-col rounded-lg cursor-pointer relative z-[10] ' onClick={() => !isBooked ? handleClick() : null}>
            ...
            {isBooked && host == userData?._id && <div className='text-purple-600 bg-white rounded-lg absolute flex items-center justify-center right-1 top-[50px] gap-[5px] p-[5px]' onClick={()=>setPopUp(true)} ><FcCancel className='w-[20px] h-[20px]' />Cancel Booking</div>}
            ...
            <div className=' w-[100%] h-[33%] py-[20px] flex flex-col gap-[2px]'>
                ...
                <span className='text-[16px] font-bold text-purple-600'>₹{rent}/day</span>
            </div>
